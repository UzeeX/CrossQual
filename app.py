...
st.caption("Internal Factor Classification & Concentration Analysis")
...
if portfolio_file and qualifying_file:
    ...
    def generate_internal_analysis(results_series, percent_series, strategy_matrix):
        ...
        top_two_percent = percent_series.iloc[:2].sum()
...
Top two strategies represent {round(top_two_percent,1)}% of the portfolio,
...
"""
    ...
    st.divider()
    st.subheader("Internal AI Analysis")

    analysis_text = generate_internal_analysis(
        results_series,
        percent_series,
        strategy_matrix
    )
